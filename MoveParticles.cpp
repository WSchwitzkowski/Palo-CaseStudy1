...
void MoveParticles(const int nr_Particles, Particle* const partikel, const float dt) {
	...
	for (int i = 0; i < nr_Particles; i++) {
		...
		for (int j = 0; j < nr_Particles; j++) {
			...
			// Abschwaechung als zusätzlicher Abstand, um Singularitaet und Selbst-Interaktion zu vermeiden
		...
		// Berechne Aenderung der Geschwindigkeit des Partikel i durch einwirkende Kraft 
		partikel[i].vx += dt * Fx;
		partikel[i].vy += dt * Fy;
		partikel[i].vz += dt * Fz;
	}
